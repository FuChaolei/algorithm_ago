...
function dfs$1(node, map, dist, m, adjArr) {
  const adjNodes = adjArr[node];
  ...
  for (let i = 0; i < adjNodes.length; i++) {
    ...
    if (isNil(nextNode)) {
      ...
      if (dfs$1(nextNode, map, dist, m, adjArr)) {
        map.set(node, adjNode);
        map.set(adjNode, node);
        return true;
      }
